...
FSM is one of the most commonly used design technique in stateful logic hardware circuits. More specifically, a hardware implementation of FSM requires a $n$-bit register comprised of $n$ flip flops to store state variables, a block of combinational circuit to determine the state, a block of sequential circuit to operate the input according to the state, and a block of combinational output circuit to forward the resulting signals, as shown in Figure TODO: Figure here.


Finite State Machine Design Methodology

Top Level Module - Connection
