...
In Figure \ref{fig:dns-data-exfil-cycle}, the malware detects a secret file from its host, hence it constructs a DNS query with the secret phrase, or more likely a \code{base64} encoded phrase to circumvent more secured firewall, as the Third-Level Domain (3LD) in the DNS query. As demonstrated in Section \ref{section:introduction-problemstatement}, the local DNS resolver would be unable to find it cache and hence this query will eventually reach \url{malicious.com}'s authoritative name server. Since the attacker has full control over \url{malicious.com}, it would then be able to retrieve the secret phrase from a highly secured host.

\begin{figure}[h!]
  \includegraphics[width=\textwidth]{docs/report/imgs/dns-data-exfil-out.png}
...
\begin{figure}[h!]
  \includegraphics[width=\textwidth]{docs/report/imgs/dns-data-exfil-cycle.png}
...
\begin{figure}[h!]
  \includegraphics[width=\textwidth]{docs/report/imgs/data-exfil-high-throughput-pcap.jpg}
...
Compared to high-throughput tunnelling, low-throughput tunnelling could be extremely hard to detect and remains overlooked within the cyber-security community \cite{nadler-2017, nadler-201936, ahmed-2020}.
